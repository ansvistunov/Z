...
public class RMLParser extends AbstractParser {
	...
	private RMLLanguageSupport xls;
	private PropertyChangeSupport support;
	private RMLTreeNode curElem;
	private RMLTreeNode root;
	

	public RMLParser(RMLLanguageSupport xls) {
		this.xls = xls;
		support = new PropertyChangeSupport(this);
		curElem = root = new RMLTreeNode("Root");
		//TODO создание парсера
	 ...
	 */
	private String getMainAttribute(Hashtable attributes) {

		//TODO возвращаем alias
		return "alias";
	...
	RMLTreeNode createTree(RSyntaxDocument doc, Proper prop){
		...
		RMLTreeNode oldParent; 
		RMLTreeNode element = null;
		while ( foo != null ){
			element = new RMLTreeNode(foo.tag);
			element.setMainAttribute(foo.getAliasOrId());
			element.setProper(foo);
			try{
				element.setStartOffset(doc.createPosition(foo.s));
				element.setEndOffset(doc.createPosition(foo.e));
			}catch(Exception e){System.out.println(e);}
			
			
			//curElem - это наш parent
			curElem.add(element);
			...
			while(e.hasMoreElements()){
				try{
					...
					HashRow oo = (HashRow)(o);
					//System.out.println("prop="+oo);
					RMLTreeNode childProper = new RMLTreeNode(oo.name.toString());
					childProper.setStartOffset(doc.createPosition(oo.s));
					childProper.setEndOffset(doc.createPosition(oo.e));
					childProper.setProper(foo);
					element.add(childProper);
				}catch(Exception ee){}
				
				//childProper.setStartOffset(oo.s);
			...
			/*if (foo!=null){
				element = new RMLTreeNode(foo.tag);
				//System.out.println(level+element);
				curElem.add(element); //добавили брата
			}*/
		}
		return element;
	...
	public ParseResult parse(RSyntaxDocument doc, String style) {
		...
		try {
			...
			Proper proper =  rml.Parser.createProper(text.toCharArray(), null);
			//proper.tag = "Root";
			createTree(doc,proper);
			
			
			/*Lexemator lex = new Lexemator(text.toCharArray());
			
			int c = lex.next(true);
			while(lex.type()!=Lexemator.LEND){
				String s = lex.as_string();
				System.out.println(s+" at "+c+" type="+lex.type());
				c = lex.next(true);
			}
			*/
			...
			e.printStackTrace();
				...
				//.createProper(doc.getText(0, doc.getLength()), null);
